...
def module_setup(request, device, app_dir, artifact_dir):
    def module_teardown():
        device.run_ssh('ls -la /var/snap/stable-diffusion/current/config > {0}/config.ls.log'.format(TMP_DIR), throw=False)
        device.run_ssh('cp /var/snap/stable-diffusion/current/config/stable-diffusion.yaml {0}/stable-diffusion.yaml.log'.format(TMP_DIR), throw=False)
        ...
        device.run_ssh('ls -la /snap/stable-diffusion > {0}/snap.stable-diffusion.ls.log'.format(TMP_DIR), throw=False)
        ...
        device.run_ssh('ls -la /data/stable-diffusion > {0}/data.stable-diffusion.ls.log'.format(TMP_DIR), throw=False)

        app_log_dir = join(artifact_dir, 'log')
        os.mkdir(app_log_dir)
        device.scp_from_device('/var/snap/stable-diffusion/common/log/*.log', app_log_dir)
...
def __log_data_dir(device):
    ...
    device.run_ssh('ls -la /data/')
    device.run_ssh('ls -la /data/stable-diffusion')


def test_storage_change_event(device):
    device.run_ssh('snap run stable-diffusion.storage-change > {0}/storage-change.log'.format(TMP_DIR))


def test_access_change_event(device):
    device.run_ssh('snap run stable-diffusion.access-change > {0}/access-change.log'.format(TMP_DIR))
